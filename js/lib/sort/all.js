...
const byType = [].concat(
  (data || [1000, 2000, 5000]).map((n) => ({ ...bubbleSortDefs.basic, count: n })),
  (data || [100000, 1000000, 3000000]).map((n) => ({ ...quickSortDefs.basic, count: n })),
  (data || [100000, 1000000, 3000000]).map((n) => ({ ...heapSortDefs.basic, count: n })),
  (data || [50000, 100000, 200000]).map((n) => ({ ...mergeSortDefs.topDown, count: n })),
  (data || [50000, 100000, 200000]).map((n) => ({ ...mergeSortDefs.bottomUp, count: n })),
);

const bySpeed = _.flatten(
  (data || [1000, 5000, 10000]).map((n) =>
    allDefs.map((def) => ({
      ...def,
      count: n,
    })),
